	...
	Collection<MonsterStatusEffect> buffs = life.getStati().values();
        ...
        for (MonsterStatusEffect buff : buffs) {
            if (buff != null && buff.getStati() != MonsterStatus.WEAPON_DAMAGE_REFLECT && buff.getStati() != MonsterStatus.MAGIC_DAMAGE_REFLECT && (!ignore_imm || (buff.getStati() != MonsterStatus.WEAPON_IMMUNITY && buff.getStati() != MonsterStatus.MAGIC_IMMUNITY && buff.getStati() != MonsterStatus.DAMAGE_IMMUNITY))) {
                ...
                mplew.writeShort((int) (buff.getCancelTask() - System.currentTimeMillis()) / 1000/*buff.getStati() == MonsterStatus.HYPNOTIZE ? 40 : (buff.getStati().isEmpty() ? 0 : 1)*/);
            }
        }
...
//	mplew.writeInt(0);
    ...
    public static byte[] applyMonsterStatus(final int oid, final MonsterStatus mse, int x, MobSkill skil, long duration, int effectDelay) {
        ...
        mplew.writeShort((int) duration / 1000/*(int) duration / 100*//*mse.isEmpty() ? 1 : 0*/); // 10 = 1秒 // might actually be the buffTime but it's not displayed anywhere
...
//        mplew.write(1); // ? v97
    ...
    public static byte[] applyMonsterStatus(final MapleMonster mons, final MonsterStatusEffect ms, long duration) {
        ...
        mplew.writeShort((int) duration / 1000/*ms.getStati().isEmpty() ? 1 : 0*/); // 持續時間 10 = 1秒 // might actually be the buffTime but it's not displayed anywhere
