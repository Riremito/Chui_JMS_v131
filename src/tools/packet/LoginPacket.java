...
//        mplew.writeInt(0);
    ...
    public static final byte[] getCharList(final String secondpw, final List<MapleCharacter> chars, int charslots) {
        ...
        for (final MapleCharacter chr : chars) {
            addCharEntry(mplew, chr, true/*!chr.isGM() && chr.getLevel() >= 30*/, false);
        }
        mplew.write(3);
        mplew.write(0);
...
//        mplew.writeLong(charslots);
    ...
    public static final byte[] getGameGuardUpdate() {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.GAMEGUARD.getValue());
        mplew.write(1);
