...
/*        if (damage == -1) {
        ...
        } else */if (damage < -1 || damage > 10000) {
            System.out.println(chr.getName() + " damage < -1 || damage > 10000");
        ...
        if (chr.getStat().dodgeChance > 0 && Randomizer.nextInt(100) < chr.getStat().dodgeChance) {
            c.announce(MaplePacketCreator.showSpecialEffect(GameConstants.GMS ? 20 : 21)); //lol
            return;
        }
...
//        final byte unk = slea.readByte(); // Added on v.82
    ...
    public static final void SpecialMove(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || chr.hasBlockedInventory() || chr.getMap() == null || slea.available() < 5/*9*/) {
            c.announce(MaplePacketCreator.enableActions());
            return;
        }
