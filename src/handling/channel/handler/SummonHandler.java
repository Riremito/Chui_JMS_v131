...
//        System.out.println(slea.toString());
        ...
        final MapleSummon summon = chr.getSummons().get(skillid);
        ...
        if (summon.isPuppet() && summon.getOwnerId() == chr.getId() && damage > 0) { //We can only have one puppet(AFAIK O.O) so this check is safe.
            ...
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.damageSummon(chr.getId(), skillid, damage, unkByte, monsterIdFrom, stance), summon.getTruePosition());
        }
        if (remove) {
            chr.cancelEffectFromBuffStat(MapleBuffStat.PUPPET);
        }
/*        final Iterator<MapleSummon> iter = chr.getSummonsReadLock().values().iterator();
        MapleSummon summon;
        boolean remove = false;
        try {
            while (iter.hasNext()) {
