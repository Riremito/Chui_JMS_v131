...
//    private final ReentrantReadWriteLock poisonsLock = new ReentrantReadWriteLock();
    ...
    private final void initWithStats(final MapleMonsterStats stats) {
        setStance(5);
        this.stats = stats;
        if ((stats.getId() >= 9300000 && stats.getId() <= 9300136) || stats.isBoss()) {
            maxhp = stats.getDefaultHp() * Randomizer.rand(15, 20) / 10;
        } else {
            maxhp = stats.getDefaultHp() * Randomizer.rand(10, 15) / 10;
        }
